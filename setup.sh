...
# ── 8a. Credentials dir + .env template ──────────────────────────────
CREDS_DIR="/home/vagrant/credentials/jarvis"
ENV_FILE="$CREDS_DIR/.env"
mkdir -p "$CREDS_DIR"

if [ -f "$ENV_FILE" ]; then
  success ".env already exists at $ENV_FILE"
else
  info "Creating .env template at $ENV_FILE..."
  cat > "$ENV_FILE" <<'ENVTEMPLATE'
# ── JARVIS environment variables ─────────────────────────────────────────────
# Generated by setup.sh. Fill in the values marked REQUIRED before running
# make docker-up or make run.
# ─────────────────────────────────────────────────────────────────────────────

# ── Secrets (REQUIRED) ───────────────────────────────────────────────────────
ANTHROPIC_API_KEY=           # Required for Claude dialogue, vision, and fusion
...
MAX_RECV_MSG_SIZE=8388608    # 8 MiB
MAX_SEND_MSG_SIZE=8388608    # 8 MiB
...
# ── Dialogue tuning ───────────────────────────────────────────────────────────
DIALOGUE_MAX_HISTORY=20
DIALOGUE_SESSION_TTL=120m
DIALOGUE_CONFIDENCE_THRESH=0.6

# ── Fusion engine ────────────────────────────────────────────────────────────
FUSION_MODEL=claude-haiku-4-5-20251001
FUSION_MAX_TOKENS=512
FUSION_CONF_IMMEDIATE=0.90
FUSION_CONF_VERIFY=0.70
FUSION_CONF_REVIEW=0.50

# ── Intelligence / RSS ───────────────────────────────────────────────────────
RSS_FEED_URLS=               # comma-separated list of feed URLs
RSS_POLL_INTERVAL=15m
KNOWLEDGE_STALE_DAYS=30
KNOWLEDGE_WEB_SEARCH_MAX_USES=10

# ── Session / Redis ──────────────────────────────────────────────────────────
SESSION_PROVIDER=memory      # memory | redis
REDIS_ADDR=redis:6379        # used when SESSION_PROVIDER=redis

# ── Security / Face analysis ─────────────────────────────────────────────────
FACE_CASCADE_PATH=           # path to pigo facefinder cascade; empty = face detection disabled
FACE_OUTPUT_DIR=             # directory for annotated face images
FACE_MIN_SIZE=65
FACE_MAX_IMAGE_BYTES=0       # 0 = server default (5 MiB)
FACE_QUALITY_THRESHOLD=7.0
FACE_CLUSTER_OVERLAP=0.25
FACE_OUTPUT_TRIANGLE_SIZE=0.22
FACE_OUTPUT_OPACITY=0.7
FACE_OUTPUT_FONT_SIZE=0

# ── Security / Threat scene ──────────────────────────────────────────────────
THREAT_VISION_ENABLED=false
THREAT_LOG_MODE=manual       # auto | manual | all
THREAT_LOG_IMAGES=false
THREAT_EVENT_DIR=            # directory for stored threat frames

# ── Alexa integration ────────────────────────────────────────────────────────
ALEXA_COOKIES_PATH=          # path to alexa-cookies.json
ALEXA_DEBUG=false
ALEXA_KEEPALIVE_INTERVAL=12h

# ── SMTP (optional — enables email features and calendar invites) ────────────
SMTP_HOST=                   # e.g. smtp.gmail.com
SMTP_PORT=                   # e.g. 587
SMTP_USER=                   # e.g. you@gmail.com
SMTP_PASS=                   # Gmail App Password (not your account password)
SMTP_TO=                     # organizer address — always receives a copy
TASK_NOTIFY_SELF_ASSIGN=false

# ── Profiling (optional) ─────────────────────────────────────────────────────
PPROF_DIR=/tmp/profiles
PPROF_INTERVAL=480m
ENVTEMPLATE
  success ".env template created at $ENV_FILE"
  warn "Open $ENV_FILE and fill in ANTHROPIC_API_KEY and TOKEN_SECRET before running make docker-up"
fi
