...
func main() {
	...
	coordinatorServer := server.New(cfg, log)
	agentv1.RegisterAgentCoordinatorServiceServer(grpcServer, coordinatorServer)

	healthSrv := health.NewServer()
	grpc_health_v1.RegisterHealthServer(grpcServer, healthSrv)
	healthSrv.SetServingStatus(
		"jarvis.agent.AgentCoordinatorService",
	...
	log.Info("JARVIS Agent Coordinator starting",
		slog.String("addr", addr),
		slog.String("service", "agent-coordinator"),
	)
