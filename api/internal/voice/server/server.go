...
// New creates a VoiceServer with the provided NLP client (in-process or remote),
// builds the STT transcriber and TTS synthesizer from config, and initialises
// the session store.
func New(cfg *config.Config, nlpClient nlpv1.NLPServiceClient, log *slog.Logger) (*VoiceServer, error) {
...
// NewWithClient creates a VoiceServer for tests using stub backends.
